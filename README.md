  ...
  checkov:
    ...
    steps:
      ...
      - name: Run Checkov with reviewdog
        uses: fulgas/reviewdog-action-checkov@v2.19.0
        with:
...
jobs:
  checkov:
    runs-on: ubuntu-latest
    container:
      image: ghcr.io/fulgas/reviewdog-action-checkov:2.19.0
    steps:
      - name: Checkout code
        uses: actions/checkout@v4
...
1. **Action Usage**: The action uses a pre-built Docker image from GHCR (`ghcr.io/fulgas/reviewdog-action-checkov:2.19.0`)
